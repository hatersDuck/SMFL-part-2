...
rect::rect() {
	...
	center.x = tangle[0].position.x + sizeA * 0.5f;
	center.y = tangle[0].position.y - sizeB*0.5f;
...
void rect::update(int w, int h) {
	move();
	rotate();
	if (checkBounce(4, w, h))
		bounce(bouX);
}
...
void rect::rotate() {
	...
	for (int i = 0, j = 0; i < 4; i++, j++) {
		...
		float x = center.x + (i < 2 ? 1 : -1) * radius * cos(tilt * t + angle[j % 2]);
		float y = center.y + (i < 2 ? 1 : -1) * radius * sin(tilt * t + angle[j % 2]);
