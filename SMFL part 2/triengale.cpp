...
triengale::triengale() {
	...
	pos.x = (tangle[0].position.x + tangle[1].position.x + tangle[2].position.x) / 3.f;
	pos.y = (tangle[0].position.y + tangle[1].position.y + tangle[2].position.y) / 3.f;

	r[0] = sqrt(pow(pos.x - tangle[0].position.x, 2) + pow(pos.y - tangle[0].position.y, 2));
	r[1] = sqrt(pow(pos.x - tangle[1].position.x, 2) + pow(pos.y - tangle[1].position.y, 2));
	r[2] = sqrt(pow(pos.x - tangle[2].position.x, 2) + pow(pos.y - tangle[2].position.y, 2));
...
void triengale::rotate() {
	...
	for (int i = 0; i < 3; i++) {
		float x = pos.x + r[i] * cos(tilt * t + angle[i]);
		float y = pos.y + r[i] * sin(tilt * t + angle[i]);
